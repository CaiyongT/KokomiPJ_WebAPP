<template>
  <div class="user-info-page">
    <!-- Ant Design 标签页组件 -->
    <a-tabs :activeKey="activeKey" @change="handleChange" :type="tabsType" :tabBarGutter="22" size="normal"
      :style="tabBarStyle">
      <!-- 动态渲染真实标签页 -->
      <a-tab-pane v-for="tab in realTabs" :key="tab.key">
        ...
        <UserSearchForm v-if="tab.userId === null" @onSearch="userId => handleSearch(tab.key, userId)" />
        <UserDetailPage v-else :userId="tab.userId" />
      </a-tab-pane>

      <!-- 特殊标签页：用于添加新标签的 '+' 按钮,最多允许5个标签页 -->
      <a-tab-pane key="__add__" :disabled="realTabs.length >= 5">
        <template #tab>
...
function handleSearch(tabKey, userId) {
